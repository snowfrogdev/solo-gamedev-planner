...
export function createInputPanel(
  ...
  container.innerHTML = `
    <div class="input-panel">
      ...
      <div class="slider-list">
        ...
        <div class="slider-group disabled">
          <div class="slider-header">
            <label for="minIncome">Minimum Pre-Tax Income</label>
            <span class="slider-value">${current.minIncome.toLocaleString()} $/year</span>
          </div>
          <input type="range" id="minIncome" min="0" max="200000" step="1000" value="${current.minIncome}" disabled>
          <span class="input-note">Coming soon</span>
        </div>
      </div>
  ...
  function emitChange(): void {
    ...
    if (isNaN(timeHorizon) || isNaN(minDev) || isNaN(targetDev)) return;
    if (timeHorizon <= 0 || minDev <= 0 || targetDev <= 0) return;
